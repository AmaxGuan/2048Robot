...
float score_toplevel_move(board_t board, int move) {
    ...
    fprintf(stdout, "Move %d: result %f: eval'd %d moves (%d cache hits, %d cache size) in %.2f seconds (maxdepth=%d)\n", move, res,
...
int find_best_move(board_t board) {
    ...
    print_board(board);
    fprintf(stdout, "Current scores: heur %.0f, actual %.0f\n", score_heur_board(board), score_board(board));
...
int ask_for_move(board_t board) {
    ...
    *validpos = 0;
    ...
    while(1) {
        ...
        fprintf(stdout, "Move [%s]? ", validstr);
